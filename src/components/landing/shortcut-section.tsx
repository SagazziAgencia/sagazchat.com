...
export const ShortcutSection = () => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
             setMessages(prev => [...prev, { 
                ...
                type: 'sent' as const, 
                isSystem: true,
                text: 'Olá! 👋 Que bom ter você aqui. A BlackConversa é especialista em automação...' 
            }]);
  ...
  return (
    <div className="min-h-screen font-sans text-white selection:bg-green-500 selection:text-white relative overflow-x-hidden flex flex-col justify-center">
      
      {/* --- Black to Green Gradient Background --- */}
      <div className="absolute inset-0 bg-gradient-to-br from-black via-[#05100c] to-[#004d40] z-0"></div>
      
      {/* --- Grid Pattern Overlay --- */}
      <div className="absolute inset-0 bg-[radial-gradient(rgba(255,255,255,0.08)_1px,transparent_1px)] [background-size:32px_32px] pointer-events-none z-0"></div>

      {/* --- Main Content Section --- */}
      <div className="relative z-10 w-full py-12 lg:py-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          {/* Changed gap-16 to gap-12 for mobile compact view */}
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            
            {/* Left Column: Direct Explanation */}
            <div className="space-y-8 animate-fadeIn text-center lg:text-left">
                
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/30 text-green-400 text-[11px] font-bold uppercase tracking-wider">
                ...
                <div className="text-lg text-slate-300 leading-relaxed max-w-lg mx-auto lg:mx-0 border-l-4 border-green-500 pl-6 text-left">
                    <p>
                ...
                {/* Stats: Stack on very small screens, row on small+ */}
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-8 justify-center lg:justify-start">
                    <button className="w-full sm:w-auto bg-green-600 hover:bg-green-500 text-white font-bold py-3.5 px-8 rounded-full shadow-lg shadow-green-900/20 transition-all transform hover:scale-105 flex items-center justify-center gap-2">
                      <UserPlus size={20} />
                      Assinar Agora
                    </button>
                    
                    <button className="w-full sm:w-auto flex items-center justify-center gap-2 text-green-400 font-bold hover:text-green-300 transition-colors group px-6 py-3.5 border border-transparent hover:bg-white/5 rounded-full">
            ...
            {/* Added scale transform for mobile to fit phone properly without overflow */}
            <div className="relative flex justify-center lg:justify-end transform scale-[0.85] sm:scale-100 origin-top sm:origin-center -mb-20 sm:mb-0">
                 
                 {/* Glow Effect behind phone */}
                 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[600px] bg-green-500/20 rounded-full blur-[100px] -z-10"></div>
                 ...
                 <div className="relative border-gray-950 bg-gray-950 border-[12px] rounded-[3rem] h-[650px] w-[320px] shadow-2xl shadow-black/50 flex flex-col overflow-hidden ring-1 ring-white/10">
                    ...
                    <div className="bg-[#1f2c34] px-4 py-3 flex items-center gap-3 z-20 shadow-md border-b border-white/5">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-emerald-700 flex items-center justify-center text-white font-bold text-sm">L</div>
                    ...
                    <div className="flex-1 bg-[#0b141a] relative overflow-y-auto p-3 space-y-4 no-scrollbar">
                         {/* WhatsApp default background pattern opacity */}
                        ...
                        <div key={msg.id} className={`flex ${msg.type === 'sent' ? 'justify-end' : 'justify-start'} animate-fadeIn relative z-10`}>
                            <div className={`max-w-[85%] rounded-lg p-2.5 text-sm shadow-sm relative ${
                                msg.type === 'sent' 
                                    ? 'bg-[#005c4b] text-white rounded-tr-none' // Dark mode sent bubble color
                                    : 'bg-[#202c33] text-white rounded-tl-none' // Dark mode received bubble color
                            }`}>
                                {msg.isSystem && (
                                    <div className="text-[9px] text-green-300 font-bold mb-1 uppercase flex items-center gap-1 opacity-80">
                                        <Zap size={10} /> Auto
                                    </div>
                                )}
                    ...
                    <div className="bg-[#1f2c34] p-2 shrink-0 z-20 flex items-center gap-2 relative">
                         ...
                         {/* Button with internal tooltip */}
                         <div className="relative">
                            <button 
                                onClick={handleSimulateCommand}
                                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-lg relative z-20 ${inputValue === '!apresentacao' ? 'bg-green-500 text-white hover:bg-green-400 shadow-green-500/20' : 'bg-[#005c4b] text-white'}`}
                            >
                                <Send size={18} />
                            </button>

                            {/* Floating Tooltip INSIDE mockup */}
                            {inputValue === '' && messages.length < 2 && (
                                <div className="absolute bottom-12 right-0 w-max bg-green-500 text-white text-[10px] font-bold px-3 py-1.5 rounded-xl shadow-xl shadow-black/50 animate-bounce z-10 border border-green-400/50">
                                    Clique aqui!
                                    <div className="absolute -bottom-1 right-3 w-3 h-3 bg-green-500 rotate-45 border-b border-r border-green-400/50"></div>
                                </div>
                            )}
                         </div>
                    </div>
